...
def guardar_vuelo_bitacora(datos_vuelo):
    """Guarda un nuevo registro de vuelo en la pestaña 'Bitacora'."""
    try:
        conn = st.connection("gsheets", type=GSheetsConnection)
        ...
        st.success("✅ ¡Vuelo registrado exitosamente en Google Sheets!")
    except Exception as e:
    ...
    if not st.session_state.authenticated:
        ...
        with st.form("vuelo_oficial_form", clear_on_submit=True):
            ...
            with col3:
                ...
                horas_totales_vuelo = round(horas_dc + horas_vs, 2)
            ...
            if btn_guardar:
                # Armando diccionario para subir a Sheets
                nuevo_vuelo_data = {
                ...
                guardar_vuelo_bitacora(nuevo_vuelo_data)
                st.cache_data.clear()
